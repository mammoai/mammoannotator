...
class CroppedImage:
    ...
    def from_raw_image(cls, raw_image:RawImage):
        ...
        crop_start = min(crop_start, image.shape[0] - image.shape[1]) # latest start to be able to get a square
        ...
        return cls(
            image=image, crop_start=crop_start, crop_end=crop_end, 
            laterality=raw_image.laterality, view=raw_image.view, 
            image_path=full_path, rotation=rotate, flip=flip
        )
...
class MRITask:
    ...
    @staticmethod
    def get_img(crops: Dict[Tuple[str,str],CroppedImage],
                lat:str, view:str, n_px:int) -> np.array:
    ...
    def from_study_folder(cls, study_path:str) -> 'MRITask':
        ...
        for i in raw_shapes[1:]:
            assert np.all(i == raw_shapes[0]), f"Image with different shape: {study_path}"
        n_px = raw_shapes[0][1]
...
if __name__ == "__main__":
    root_folder = "/home/fer/projects/mammoannotator/test/data/mri/"
    task_json_path = "/home/fer/projects/mammoannotator/test/tasks/mri/tasks.json"
    tasks = list(MRITask.from_root_folder(root_folder))
    for task in tasks:
        task.make_url("/home/fer/projects/mammoannotator/test/data/", "http://localhost:8000")
    with open(task_json_path, "w") as file:
        tasks = [t.as_dict() for t in tasks]
        file.write(json.dumps(tasks, indent=4))

    # study_path = "/home/fer/projects/mammoannotator/test/data/mri/54685a6b43326b4777315271482f49505a4d654430413d3d/Fo3Qxe1aNysvwql1vNG8FSiFSLASHRf5aQjcppJwUqTSFSLASH4lgGpKoUX5flV4uHMyVnU1IT/"
    #image_path = "/home/fer/projects/mammoannotator/test/data/mri/54685a6b43326b4777315271482f49505a4d654430413d3d/Fo3Qxe1aNysvwql1vNG8FSiFSLASHRf5aQjcppJwUqTSFSLASH4lgGpKoUX5flV4uHMyVnU1IT/901003_eTHRIVE_Tra_1-6_dyn_mip_l_Ax.jpeg"
    # for image_path in list_files(study_path, ".jpeg"):
    #     raw_image = RawImage.from_path(image_path)
    #     cropped_image = CroppedImage.from_raw_image(raw_image)
    # MRITask.from_study_folder(study_path)
